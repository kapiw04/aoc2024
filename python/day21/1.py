...
def code_on_keypad(code, depth, keypad, current_position=None):
    if not code:
        return 0  
    ...
    if depth == 0:
        return len(move_sequence) + code_on_keypad(code[1:], depth, keypad, (target_x, target_y)) + 1
    ...
    for permutation in set(permutations(move_sequence)):
        ...
        permutation_lengths.append(
            code_on_directional(list(permutation) + ["A"], depth - 1)
        )
    length = min(permutation_lengths)

    return length + code_on_keypad(code[1:], depth, keypad, (target_x, target_y))

code_on_directional = partial(code_on_keypad, keypad=dir_keypad)
code_on_num = partial(code_on_keypad, keypad=num_keypad)

s = 0

for code in codes:
    result = code_on_num(code, 2, current_position=(2, 3))
    s += result * int(code[:-1])
